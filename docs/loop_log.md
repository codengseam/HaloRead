...
## 沉淀记录｜《锻炼养生课》专栏（2026-06-24）

### 任务概要
生成融合传统导引与现代运动科学的养身专栏，38章，每章2000-3500字，故事化叙事，核心目标"通过锻炼养好身体"。

### 执行流程
1. **专家团三视角并行评审方案**（生成前）：故事性/科学性/养生目标三agent并行评审章节方案，全部给出"需修改"，共识：健美减脂挤压养身主线、缺激素/线粒体/睡眠/免疫/运动的度等养身基石、动作教学章过多、标题去术语留悬念。
2. **方案重构**：砍肌肥大/平台期等健美导向章，新增激素/线粒体/睡眠/免疫/运动的度等养身核心章，传统动功前移加重，全部故事开场。
3. **8agent并行生成**：每agent 3-5章，共享写作规范+权威素材库+纠偏清单。
4. **3agent并行质检**：可读性/科学性/引用合规三视角，覆盖38章。
5. **3agent并行修复**：P0事实错误/P1句式与重复/P2引用格式。
6. **二次句式修复**：对超标章节单独降密度。
7. **git提交+push**：39文件3825行，rebase后推送成功。

### 新共性问题
1. **"不是X，是Y"句式再次系统性超标**：38章初版106处，首轮修复降至67，二轮修复后仍有个别章4-6处。这是AI写作的顽固倾向（与睡眠课记录一致）。**建议**：quality.py增加单章该句式计数检测，超3处自动标记；写作规范明确"每章≤2处"硬约束。
2. **跨章案例重复**：施瓦辛格/麦吉尔/BDNF在多章重复展开。根因：并行agent各自独立，不知道其他agent用了同一案例。**建议**：写作规范增加"核心案例归属章"表，标注每个主要案例只在哪一章主场出现，其他章引用须一笔带过。
3. **并行生成的事实错误类型**：地名错误（阿德莱德→悉尼）、来源归属错置（Norton→Leibel）、绝对化表述（"没有副作用"）。这些是单agent难以自查的。**建议**：质检阶段必须包含Web核验关键事实（人名机构地名、研究归属）。
4. **传统养生边界执行良好**：三层边界（可实证/可观察明局限/文化不硬套科学）在29-33章全部合规，仅格式标注不统一。**可复用**：三层边界模型适合所有中西结合内容。

### 规则/工具更新建议
- **content-quality.md**：增设"科普/养身类内容"适配分支——真实性子项替换为"科学共识核验+权威研究归属核验+安全警示核验"；保留顺序/引用维度。
- **quality.py**：增加"不是X，是Y"单章计数检测（阈值3处/章）+ 跨章案例重复检测。
- **写作规范模板**：增加"核心案例归属章"表 + 句式硬约束 + 传统养生三层边界模板。

### 可复用资产
- **专家团前置评审模式**：生成前用3agent（故事/科学/目标）评审方案，能提前发现结构性问题，避免生成后大改。比生成后质检更省成本。
- **8agent并行生成+3agent并行质检+3agent并行修复**的三阶段模式，适合40章级大型专栏。
- **养身专栏的权威素材库与纠偏清单**（拉伸/HIIT/局部减脂/热量赤字/过度训练/免疫开窗/传统边界），可复用于后续健康类专栏。
