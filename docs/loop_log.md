...
### 待办（下一 Loop）
1. **会话A+B 已合并**：`workflow.get_required_sections` 同款 PyYAML bug（BUG-030 待办）需修。
2. **边链按桶裁剪 specialist**：解决 modern/knowledge 桶调用不需要的 specialist 产空段问题，为开启 modern/knowledge soul injection 扫清障碍（design.md §10.6、阶段4 沉淀"占位 agent 与边链裁剪的边界"）。
