...
- [2026-07-03 重庆备婚全流程手册专栏生成（2026-07-03，47 篇 modern 桶 + 8 撰写+5 质检+7 修复 三波并行调度 + 专名拼写与术语混用两类复发问题）](loop_log/2026-07.md#loop-20260703-5a0976)
...
| `#content_quality`（质检规则/灵魂注入/标题评分） | 32 | — |
| `#book_structure`（排序/校验/命名/去重/双源同步） | 30 | — |
| `#deployment`（GitHub Pages/魔搭/.nojekyll/SW） | 5 | — |
| `#soul_injection`（灵魂注入/章回体/总编Agent） | 3 | — |
| `#ai_course`（专栏批量生成 / subagent 结果丢失） | 18 | — |

> 共 59 条沉淀，按月份分片存储于 `docs/loop_log/`。
