...
### 测试覆盖
- `tests/test_workflow_archetype.py`：49 项契约测试（build_workflow 参数 / get_required_sections 纯函数 / config section_templates / editor SECTION_TEMPLATES / stub 按 archetype 生成段数 / quality_node 闭包路由 / soul injection 按桶路由 + 边链 / quality_router 条件边直测 / _soul_injection_for_archetype 开关组合 / BUG-029 fiction 回归）。
- 三件套：`check_book_structure.py --strict` 0 问题；pytest 259 passed 15 skipped（忽略 4 个 langgraph 依赖测试文件）；`run_regression_suite.sh` 18/18。
- 专家团三视角评审（架构/测试/规则）首轮均分 7，修复 P0-1（fiction 跨层崩溃）+ P1-2~5（条件边/纯函数/边链/死字段）后复评。
