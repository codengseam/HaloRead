...
## 2026-06-24｜饮食养生课 网文级阅读性增强专项 Loop

### 背景
上一轮修复后整体均分 92.8，距 95+ 目标差 2.2 分，短板在可读性（27-28/30）。启动专项：对 90-92 分的 20 章重写开篇故事，加场景/对话/戏剧转折，冲击 95+。

### 执行路径
1. **识别**：20 章（01/02/03/04/05/06/08/11/13/16/18/20/22/24/25/30/33/37/40/41）
2. **重写**（5 个并行 Agent）：每章只重写"一、讲清楚"段，400-600 字，网文小说笔法
3. **复评**（5 个并行 Agent + 1 个验证 Agent）：20 章复评 + 22 章保持验证 + 2 处重复直补

### 重写效果
| 指标 | 重写前 | 重写后 |
|---|---|---|
| 20 章可读性均分 | 27-28/30 | 29-30/30 |
| 20 章总分均分 | 91 | 96.4 |
| 整体 42 章均分 | 92.8 | 94.2 |

### 关键修复
- 饮食课33：开篇与第三节"小周换路子"近乎逐字重复 → 开篇简化、第三节保留完整版
- 饮食课20：开篇爸爸台词与讲原理段"血糖嗖地上"重复 → 讲原理段删去重复的血糖描述

### 最终评分
- **42 章全部达 90+**
- **整体均分 94.2**（20 章重写后 96.4 + 22 章保持 92.0，加权）
- 可读性维度：20 章达 29-30，22 章保持 27-28
- P0 真实性问题：零

### 沉淀经验
1. **开篇故事是可读性杠杆**：只重写"一、讲清楚"段（400-600 字），不动其余 6 段，即可把可读性从 28 推到 29-30，投入产出比极高。
2. **网文级三要素**：场景（时间/地点/感官）+ 对话（2-3 轮真实对话）+ 戏剧转折（意外/冲突/反转），缺一不可。最佳案例：饮食课08（急诊室凌晨三点）、饮食课22（冰镇西瓜半夜拉肚子）、饮食课40（保健品逐项拆解）。
3. **重写易引入新重复**：重写开篇时把原讲原理的内容戏剧化进了对话，但讲原理段未同步去重，导致章节级重复。修复时需同步检查"二、讲原理"是否与新开篇重复。
4. **人物对话要见性格**：最佳对话不是信息传递，而是性格碰撞。如饮食课40 王医生逐项拆解保健品、李姐层层退守；饮食课33 小周拍桌"我饿啊"。
5. **95+ 门槛**：整体均分 94.2，距 95 差 0.8。剩余 22 章可读性仍 27-28，若全部重写开篇可破 95。但边际收益递减，当前质量已可发布。

### 待优化项
- 22 章未重写开篇（07/09/10/12/14/15/17/19/21/23/26/27/28/29/31/32/34/35/36/38/39/42），可读性 27-28。若需冲击 96+，可启动第二轮开篇重写。当前 94.2 已达"优秀·可直接发布"标准。
