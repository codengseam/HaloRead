...
|---|---|---|
| `book_structure`（双源同步 3 次 / 排序校验已固化 / 命名去重反复 / 跨章 chapter_sort 盲区） | 12 | no（双源同步未入） |
| `reader_interaction`（沉浸模式反复修复 4 次 / 防横屏已入 / Hero 移动端瘦身） | 11 | no（双源同步未入） |
| `content_quality`（质检规则分化 / 灵魂注入 / 标题评分 / 写作规则分工 / 医学术语中文化策略） | 7 | no |
...
| `ai_course`（专栏批量生成 / subagent 结果丢失 / 富贵病手册 5+3 两波并行） | 3 | no |
