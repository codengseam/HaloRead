...
## 2026-06-24｜饮食养生课 42 章多专家团质检与修复 Loop

### 背景
饮食养生课专栏（42 章，融合中医饮食智慧与现代营养科学）需经多专家团质检，单章≥90、整体目标 95+。

### 执行路径（路径 A：Task 工具并行子代理）
LLM_API_KEY 未配置，Python 引擎不可用，全程用 Task 工具启动并行子代理模拟专家团。

### Loop 三轮
1. **初评（10 个并行 Agent）**：42 章分两批，每批 5 Agent，按四维度（真实性40/可读性30/顺序15/引用15）打分。初评均分 87.5，9 章不合格（<85）。
2. **修复（5 个并行 Agent）**：按 P0→P1→P2 优先级精准修复，不重写整章，用 Edit 替换。
3. **复评（5 个并行 Agent + 直补）**：28 章复评 + 14 章快速验证 + 5 处残留直补修复。

### 关键发现与修复
| 问题类型 | 典型案例 | 修复方式 |
|---|---|---|
| 古籍引文张冠李戴 | 05章《本草纲目》引文实出陈藏器《本草拾遗》且论糯米非精白米 | 删除错误引文，改转述"大意据" |
| 研究对象误植 | 07章 Tim Spector"双胞胎女儿"实为 TwinsUK 队列研究对象 | 改"研究过一对同卵双胞胎" |
| 案例存疑 | 08章"Lisa护士"未见于 Panda 著作，可考案例为消防员 Randy | 替换为可考案例 |
| 编造来源 | 13/23/25章"美国追踪研究""真实案例""日本便当研究"无出处 | 改泛指或删除 |
| 数据错误 | 27章"一家三口一勺半"应为"七勺半"；34/33章"四条核心"实列五条 | 直接修正 |
| 中医未标注 | ~15 章引用古籍未标"传统养生参考" | 统一加标注 |
| 术语硬套 | 7 章用"底层逻辑/底层规律" | 替换为"道理/关键/窍门" |
| Wansink 争议 | 4 章引用其研究未交代 2018 撤稿事件 | 加注说明 |
| 专家归属松 | 夏萌/范志红无具体出处 | 改泛指"临床营养师普遍观察到" |
...
### 沉淀经验
1. **古籍引文是真实性重灾区**：AI 生成时易把出处、对象、作者张冠李戴。规则已要求"记不住原句宁可不引"，但执行时仍需逐条核验。建议在 content-quality.md 增加"古籍引文必须联网核验出处+对象"的强制项。
2. **"传统养生参考"标注易遗漏**：中医内容散落各章，批量生成时标注不统一。建议在生成 prompt 中固化"凡引古籍必加（传统养生参考）"。
3. **Wansink 类争议学者需统一处理**：一次生成中多处引用同一争议学者时，应在首次出现时加注，后续不再重复。本次 4 章分别加注，略冗余，可优化为"首次加注+后续泛指"。
4. **"底层逻辑"类术语反复出现**：虽规则已禁用，但 7 章仍出现，说明生成 prompt 未有效约束。建议在 rules.md 的"不做"清单中加粗强调。
5. **路径 A 效果评估**：Task 工具并行子代理（5 个/批）在无 API Key 时可替代 Python 引擎，评审质量接近但耗时较长（2 轮×5 Agent）。若需真并行，仍应配置 LLM_API_KEY 走路径 B。

### 待优化项
- 整体均分 92.8，距 95+ 目标差 2.2 分。主要短板在可读性（27-28/30），需增强故事场景与对话方能突破。后续可启动"网文级阅读性增强"专项 Loop。
