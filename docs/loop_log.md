...
## 2026-06-23｜睡眠与精力修复课专栏（40章·子类·非讲书）

### 背景
用户要求生成融合传统养生与现代睡眠科学的精力修复专栏，40章×2000-3500字，属"子"类（医家养生），非讲书笔记。这是项目首次生成非历史叙事类专栏，暴露了规则体系对科普/实操类内容的适配缺口。

### 流程
1. **重述需求+设计40章方案**：7模块（开篇/底层逻辑/夜间优化/日间修复/冥想呼吸/场景方案/误区体系）。
2. **专家团三视角并行评审**（路径A增强版，3个Task子代理）：架构师/测试/规则，各出"有保留通过"意见。
3. **按评审意见定稿**：模块2重排（环境→仪式→入睡）、模块4重排（走神冥想前移）、统一目录名、补健康安全边界+权威来源白名单+不编造硬约束+语言风格约束。
4. **8个生成agent分2批并行**：每agent写3-7章，各先读`_写作指南.md`再写。
5. **4个质检agent并行**：每agent查10章，按四维度（真实性/可读性/顺序/引用）报告。
6. **4个修复agent并行**：按质检报告逐章修复P0/P1/P2问题。
7. **程序化验证**：grep确认白名单外来源清除、免责声明统一、句式降量。
8. **git提交+push**：42文件4610行，rebase后推送成功。

### 新共性问题
1. **白名单外来源是最高频P0**：8个生成agent中有6个不约而同引用了Herbert Benson《放松反应》、NASA午睡研究、Oppezzo&Schwarz斯坦福研究等非白名单来源。根因：写作指南只列了"允许用什么"没列"禁止用什么"，agent遇到白名单外的知名研究时倾向于引用。**建议**：写作指南增加"常见但不在白名单的来源"黑名单。
2. **"不是X而是Y"句式系统性超标**：13/40章超过4处阈值。这是AI写作的深层倾向，仅靠"不滥用"软约束不够。**建议**：quality.py增加单章该句式计数检测，超4处自动标记。
3. **免责声明格式不统一**：不同agent用了加粗/斜体/简短/完整四种格式。**建议**：写作指南提供免责声明逐字模板，agent直接复制。
4. **content-quality.md质检维度与科普内容错配**：真实性子项"史实核验/名家点评（司马光臣光曰）"完全不适用于睡眠医学内容。**建议**：content-quality.md增设"科普类内容"适配分支，真实性=医学共识核验+权威研究核验。

### 规则/工具更新
- **无需更新rules.md**：该规则明确仅适用于讲书笔记，科普专栏不套用五段结构。
- **建议更新content-quality.md**：增设科普类内容质检适配分支（真实性子项替换为医学共识核验+权威研究核验；顺序子项替换为学习递进顺序）。
- **建议更新写作指南模板**：增加来源黑名单、免责声明逐字模板、单章句式计数上限。
- **无需更新quality.py**：本次为内容生成，非代码改动；但建议后续扩充"不是X而是Y"单章计数检测。

### 可复用资产
- `output/睡眠与精力修复课/_写作指南.md`：完整的非讲书类专栏写作指南模板（含硬约束/白名单/语言风格/引用规范/frontmatter/去重对照），可复用于后续科普/实操类专栏。
- **多agent分批并行生成模式**：8agent×5章/批×2批，每agent先读指南再写，适合40+章大型专栏。
- **质检-修复分离模式**：4质检agent（只读不写）+4修复agent（按报告改），避免质检与修复混在同一agent导致遗漏。
