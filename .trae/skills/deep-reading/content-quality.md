...
- 史料层累必须交代（如马王堆帛书对苏秦年代的修订）。自动检测见 `consistency-rules.md §2.5`（争议人物清单：苏秦/鬼谷子/老子/孙子，正文出现 ≥3 次且上下文无层累提示词则报 P1）。
...
### 5.4 正文引用 ↔ 文末来源双向匹配（BUG-045）

**要做：**

- 正文行内引用 `——《书名·篇名》` 的，文末 `## 参考来源` 必须能找到对应书名（篇名可省略）。
- 文末列出的来源，正文中至少有一处行内引用或对应内容锚点（避免堆砌未引用的来源凑数）。

**不做：**

- 不在正文引了《XX》但文末连书名都不列（书名硬漏，P0）。
- 不在正文引了《XX·YY》但文末《XX》下未列《YY》篇名（篇名缺失，P1，可争议——讲书文末常省略篇名只写书名）。
- 不在文末堆砌大量正文从未提及的来源（文末多列未引，P2 提示）。

**匹配策略**（分级，不照搬论文级 DOI/格式校验）：
...
**实现**：`src/utils/quality.py:check_citation_bidirectional` 三级匹配，复用 `src/utils/sources.py:extract_references_structured` 解析文末结构化文献。仅匹配破折号引导的尾注式引用 `——《X·Y》`（讲书文体标准格式，见 §5.2），不匹配「《X》里讲过」式内嵌引用（与文末来源对应关系弱，且 §5.2 行内密度规则已覆盖）。
