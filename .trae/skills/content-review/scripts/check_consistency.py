...
from src.utils.consistency import check_consistency, format_consistency_report, ConsistencyIssue
...
def check_file(path: Path, archetype: str = "narrative") -> tuple[list[ConsistencyIssue], str]:
    """检测单个文件，返回 (问题列表, 文件相对路径)。"""
    content = path.read_text(encoding="utf-8")
    ...
    try:
        ...
        rel = str(path)
    return report.issues, rel


def check_dir(directory: Path, glob: str = "**/*.md") -> list[tuple[str, list[ConsistencyIssue]]]:
    ...
    for md_path in sorted(directory.glob(glob)):
        ...
        if not md_path.is_file():
            continue
        try:
            # 从 frontmatter category 解析 archetype，路径启发式兜底
            archetype = _detect_archetype(md_path)
            issues, rel = check_file(md_path, archetype)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--archetype", default="narrative", help="形态范式桶（narrative/modern/knowledge）")
    ...
    if args.file:
        if args.file == "-":
            ...
            report = check_consistency(content, archetype=args.archetype)
            ...
            if not path.exists():
                ...
                return 1
            issues, rel = check_file(path, args.archetype)
            all_results.append((rel, issues))
            from src.utils.consistency import check_consistency as _cc
            report = _cc(path.read_text(encoding="utf-8"), archetype=args.archetype)
            report_text = format_consistency_report(report)
    else:
        ...
        if not dir_path.exists():
            ...
            return 1
        all_results = check_dir(dir_path, args.glob)
        # 汇总
