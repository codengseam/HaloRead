...
from src.utils.quality import _cn_to_int, _strip_punct_for_char_count
...
def _strip_frontmatter(content: str) -> str:
    """去掉 YAML frontmatter，返回正文（与 content_quality.py 单一信源）。"""
...
def check_same_event_diff_value(content: str) -> List[ConsistencyIssue]:
    ...
    for bm in battle_pattern.finditer(body):
        ...
        window_start = bm.end()
        window_end = min(len(body), bm.end() + 15)
        window = body[window_start:window_end]
        for tm in troop_pattern.finditer(window):
            raw = tm.group(1)
...
_FLASHBACK_MARKERS = re.compile(
    r"此前|之前|回顾|回过头|当年早些时候|更早|在此之[前先]|上一年|前一年|三年前|两年前|一年前|多年前|早[在先]"
)
...
def check_consistency(content: str, archetype: str = "narrative") -> ConsistencyReport:
    ...
    - modern/knowledge：跳过时间线倒置（现代文无年份序列），其余仍检
    """
    issues: List[ConsistencyIssue] = []
